        ...
        public Exception CreateExceptionForFailedLoginAttempt(AbpLoginResultType result, string usernameOrEmailAddress, string tenancyName)
        {
            switch (result)
            {
                ...
                case AbpLoginResultType.InvalidPassword:
                    return new UserFriendlyException(L("InvalidUserNameOrPassword"), L("InvalidUserNameOrPassword"));
                case AbpLoginResultType.InvalidTenancyName:
                    return new UserFriendlyException(L("ThereIsNoTenantDefinedWithName{0}", tenancyName), L("ThereIsNoTenantDefinedWithName{0}", tenancyName));
                case AbpLoginResultType.TenantIsNotActive:
                    return new UserFriendlyException(L("TenantIsNotActive", tenancyName), L("TenantIsNotActive", tenancyName));
                case AbpLoginResultType.UserIsNotActive:
                    return new UserFriendlyException(L("UserIsNotActiveAndCanNotLogin", usernameOrEmailAddress), L("UserIsNotActiveAndCanNotLogin", usernameOrEmailAddress));
                case AbpLoginResultType.UserEmailIsNotConfirmed:
                    return new UserFriendlyException(L("UserEmailIsNotConfirmedAndCanNotLogin"), L("UserEmailIsNotConfirmedAndCanNotLogin"));
                case AbpLoginResultType.LockedOut:
                    return new UserFriendlyException(L("UserLockedOutMessage"), L("UserLockedOutMessage"));
